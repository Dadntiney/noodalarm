...
- Een Supabase Edge Function `post-update-digest`, dagelijks om 08:00 UTC
  ...
  vriendelijk bericht en post dat in **élke** `alarm_group` (iedere
  gebruiker ziet het in Berichten — alleen als er echt iets nieuws is,
  geen bericht op stille dagen). Zet daarna `announced_at` op die rijen.
- Bron van de edge function staat in `supabase/functions/post-update-digest/`.
  Secrets: service role + optioneel `DIGEST_SENDER_ID` (fallback-afzender).
  Het bericht **moet** een niet-lege `body` hebben
  (`alarm_messages_body_check`).
